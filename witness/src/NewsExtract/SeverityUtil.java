...
public class SeverityUtil {
	...
	public int findSeverity(String doc) {
		...
		for (int i = 3; i < doc.length() - 10; i++) {
			if (doc.charAt(i) == '死' || doc.charAt(i) == '丧') {
				int d = 0;
				if (doc.charAt(i-1) == '人') {
					d = 1;
				}
			...
			} else if (doc.charAt(i) == '伤') {
				int d = 0;
				if (doc.charAt(i-1) == '人')
					d = 1;
				else if (doc.charAt(i-1) == '轻' || doc.charAt(i-1) == '重' || doc.charAt(i-1) == '受') {
					if (doc.charAt(i-1) == '人') {
						d = 2;
					}
				}
				...
				if (s >= 1 && ans < 1 && t.contains("轻"))
					ans = 1;
				else if (s >= 1 && ans < 2)
					ans = 2; 
			} else if (doc.substring(i, i+2) == "昏迷") {
				if (doc.contains("死") || doc.contains("去世") || doc.contains("丧生")) {
					if (ans < 3)
						ans = 3;
				} else {
					if (ans < 2)
			...
			} else if (doc.substring(i, i+2) == "截肢") {
				if (doc.contains("死") || doc.contains("去世") || doc.contains("丧生")) {
					if (ans < 3)
						ans = 3;
				} else {
			...
			} else if (doc.substring(i, i+2) == "特大") {
				if (doc.substring(i+2, i+6).contains("事故")) {
					ans = 4;
				}
			} else if (doc.substring(i, i+2) == "重大") {
				if (doc.substring(i+2, i+6).contains("事故")) {
					if (ans <3)
						ans = 3;
				}
			} else if (doc.charAt(i) == '亡') {
				String ngram = "";
				if (i >= 5)
					ngram = doc.substring(i-5, i);
				else
			...
			} else if (doc.charAt(i) == '尸') {
				if (ans < 3)
					ans = 3;
			} else if (doc.charAt(i) == '命') {
				String ngram = "";
				if (i >= 8)
					ngram = doc.substring(i-8, i);
				else
					ngram = doc.substring(0,i);
				if (ngram.contains("失") || ngram.contains("丧") ||ngram.contains("殒"))
					if (ans < 3)
						ans = 3;
			}
		}
