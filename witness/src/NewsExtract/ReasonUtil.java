...
public class ReasonUtil {
	private static ReasonUtil _instance = null;
	
	public static ReasonUtil getInstance() {
		if (_instance == null) {
			_instance = new ReasonUtil();
		}
		return _instance;
	}
	protected ReasonUtil() {
	}
	
	public boolean hasAlcohol(String doc) {
		int conf = 0;
		for (int i = 0; i < doc.length() - 10; i++) {
			if (doc.substring(i,i+2).equals("酒精") || doc.substring(i,i+2).equals("饮酒")) {
				String ngramPrev = "";
				if (i > 5)
	...
	public boolean hasThrottle(String doc) {
		int conf = 0;
		for (int i = 0; i < doc.length() - 10; i++) {
			if (doc.substring(i,i+2).equals("油门")) {
				String ngramPrev = doc.substring(i+2,i+7);
				if (i > 5)
					ngramPrev = ngramPrev + doc.substring(i-5, i);
				
				if (ngramPrev.contains("刹车")) {
					conf++;
				}
			}
		}
	...
	public boolean hasVisible(String doc) {
		int conf = 0;
		for (int i = 0; i < doc.length() - 10; i++) {
			if (doc.substring(i,i+3).equals("能见度")) {
				String ngramPrev = doc.substring(i+3,i+8);
				if (i > 5)
					ngramPrev = ngramPrev + doc.substring(i-5, i);
				
				if (ngramPrev.contains("不到") ||ngramPrev.contains("低") || ngramPrev.contains("小于")) {
					conf++;
				}
			}
		}
	...
	public boolean hasOverload(String doc) {
		int conf = 0;
		for (int i = 0; i < doc.length() - 10; i++) {
			if (doc.substring(i,i+2).equals("超载")) {
				String ngramPrev = "";
				if (i > 5)
					ngramPrev = doc.substring(i-5, i);
			...
			} else if (doc.substring(i, i+1).equals("载")) {
				String ngramAfter = doc.substring(i+1,i+6);
				if (ngramAfter.indexOf("超") >= 0) {
					conf++;
				}
			}
		}
	...
	public boolean hasTurnover(String doc) {
		int conf = 0;
		for (int i = 0; i < doc.length() - 10; i++) {
			if (doc.substring(i,i+1).equals("翻")) {
				String ngramPrev = doc.substring(i+3,i+8);
				if (ngramPrev.contains("下") ||ngramPrev.contains("入") || ngramPrev.contains("到")) {
					conf++;
				}
			} else if (doc.substring(i, i+2).equals("侧翻")) {
				conf++;
			}
		}
		return conf>0;
