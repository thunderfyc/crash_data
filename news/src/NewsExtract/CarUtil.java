...
public class CarUtil {
	private static CarUtil _instance = null;
	
	public static CarUtil getInstance() {
		if (_instance == null) {
			_instance = new CarUtil();
		}
		return _instance;
	}
	protected CarUtil() {
	...
	public boolean hasTruck(String doc) {
		int conf = 0;
		for (int i = 0; i < doc.length() - 10; i++) {
			String t3 = doc.substring(i, i+3);
			if (t3.equals("土渣车") || t3.equals("土方车") || t3.equals("泥头车") ||
				t3.equals("渣土车") || t3.equals("运渣车")) {
				conf++;
			}
		}
		return conf>0;
	...
	public boolean hasTaxi(String doc) {
		int conf = 0;
		for (int i = 0; i < doc.length() - 10; i++) {
			if (doc.substring(i,i+3).equals("出租车")) {
				conf++;
			}
		}
		return conf>0;
	...
	public boolean hasBus(String doc) {
		int conf = 0;
		for (int i = 4; i < doc.length() - 10; i++) {
			if (doc.substring(i,i+2).equals("公交")) {
				conf++;
			}
		}
		return conf>0;
	...
	public boolean hasSchool(String doc) {
		int conf = 0;
		for (int i = 0; i < doc.length() - 10; i++) {
			if (doc.substring(i,i+2).equals("校车")) {
					conf++;
			}
		}
		return conf>0;
	...
	public boolean hasDistant(String doc) {
		int conf = 0;
		for (int i = 0; i < doc.length() - 10; i++) {
			if (doc.substring(i,i+2).equals("长途")) {
					conf++;
			}
		}
		return conf>0;
